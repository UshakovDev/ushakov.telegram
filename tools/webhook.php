...
if (mb_stripos($text, '/start') === 0 || preg_match('~^[a-zA-Z]{2}-\d+-[a-f0-9]{32,}+$~', trim($text))) {
    ...
    if (mb_stripos($text, '/start') === 0) {
        $payload = trim(mb_substr($text, 6)); // убрать '/start'
    } else {
        $payload = trim($text);
    }
    ...
    if ($ok) {
        ...
        $sqlIns = "INSERT INTO {$table} (SITE_ID, USER_ID, CHAT_ID, TG_USERNAME, CONSENT, CREATED_AT)
            VALUES ('{$siteIdSql}', {$userIdInt}, {$chatId}, '{$tgUsernameSql}', 'Y', NOW())
            ON DUPLICATE KEY UPDATE CHAT_ID = VALUES(CHAT_ID), TG_USERNAME = VALUES(TG_USERNAME), CONSENT='Y'";
